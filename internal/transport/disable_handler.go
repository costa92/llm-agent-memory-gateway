...
func (api *API) handleDisableMemory(w http.ResponseWriter, r *http.Request) {
	...
	var req httpapi.DisableMemoryRequest
	if err := decodeJSON(r, &req); err != nil {
		httpapi.WriteError(w, RequestIDFromContext(r.Context()), err)
		return
	}

	resp, err := api.service.DisableMemory(r.Context(), authScope, r.PathValue("memory_id"), req)
	if err != nil {
		httpapi.WriteError(w, RequestIDFromContext(r.Context()), err)
		return
	}
